...
setwd('C:/Users/arikh/OneDrive - Virginia Tech/Smithsonian Projects/VaccinationGlyphus/MicrobiomeAnalyses/glyphus_16s_analyses_figure')

```

```{r load libraries}
if (!require("pacman")) install.packages("pacman")
pacman::p_load(readr, dplyr,tidyverse, lme4, lmerTest, car, patchwork, ggpubr, rcartocolor, emmeans, stats, tibble, jtools,
 DHARMa, ggplot2, gtable, lemon, phyloseq, vegan, Biostrings, dada2, DECIPHER, phangorn, ape, permute, ecole, picante, microbiome, pairwiseAdonis)
...
# Extract metadata
meta <- data.frame(sample_data(g16F4Rare))

# Create T3
meta <- meta %>%
  mutate(
    T3 = case_when(
      X2ndBdExp == "No"  & Treatment == "UTA" ~ "untreated",
      X2ndBdExp == "No"  & Treatment == "UVC" ~ "mock-mock",
      X2ndBdExp == "Yes" & Treatment == "UV"  ~ "mock-Bd",
      X2ndBdExp == "No"  & Treatment == "VC"  ~ "Bd-mock",
      X2ndBdExp == "Yes" & Treatment == "V"   ~ "Bd-Bd",
      TRUE ~ NA_character_
    ),
    T3 = factor(
      T3,
...
sample_data(g16F4Rare) <- sample_data(meta)
...
write.csv(dat_pc2, "final_meta.csv")
...
## Within the untreated control and mock-mock groups, is there a noticeable difference between days in terms of microbiome composition when using blocks to account for individual frog ID as a random effect?
```{r reviwer question about day}

#Added by AMH 7/21/2026

#Unvaccinated control (UVC)
#Untreated control (UTC)

# Subset phyloseq to untreated and mock-mock controls only
controls_phy <- subset_samples(g16F4Rare, T3 %in% c("untreated", "mock-mock"))

# Clean metadata to avoid duplicate labels and order the exp days
meta_0 <- data.frame(sample_data(controls_phy)) %>%
  dplyr::select(
    -dplyr::any_of(c("Axis.1", "Axis.2", "Axis.3"))) %>%
  droplevels() %>%
  mutate(T3 = factor(T3,
      levels = c("untreated", "mock-mock")),
    FINAL.EXPERIMENT.DAY = factor(FINAL.EXPERIMENT.DAY, levels = c("day-1-E1","day46-E1","day118-E1","day14-E2","day29-E2")))

# Preserve sample names as metadata row names
rownames(meta_0) <- sample_names(controls_phy)

# Put cleaned up metadata back into the phyloseq object
sample_data(controls_phy) <- phyloseq::sample_data(meta_0)

#Recalculate Bray-curtis and Jaccard distances for controls and mock-mock only
bray <- phyloseq::distance(
  controls_phy,
  method = "bray")

jacc <- phyloseq::distance(
  controls_phy,
  method = "jaccard")

#extract meta data for the downstream permanovas
meta <- data.frame(sample_data(controls_phy)) %>% 
  mutate(FINAL.EXPERIMENT.DAY = factor(FINAL.EXPERIMENT.DAY, levels = c("day-1-E1","day46-E1","day118-E1","day14-E2","day29-E2")),
    T3 = factor(T3,levels = c("untreated", "mock-mock")))


#Subset metadata for untreated only
meta_u <- meta %>% filter(T3 == "untreated") %>% droplevels()

#Ensure rownames align
keep_u <- rownames(meta) %in% rownames(meta_u)

#Calculate Bray-Curtis and Jaccard distances
bray_u <- as.dist(as.matrix(bray)[keep_u, keep_u])
jacc_u <- as.dist(as.matrix(jacc)[keep_u, keep_u])

#Subset metadata for mock-mock only
meta_m <- meta %>% filter(T3 == "mock-mock") %>% droplevels()

#Ensure rownames align, calculate distances like above
keep_m <- rownames(meta) %in% rownames(meta_m)
bray_m <- as.dist(as.matrix(bray)[keep_m, keep_m])
jacc_m <- as.dist(as.matrix(jacc)[keep_m, keep_m])


# Permanovas with Dynamic blocking to compare BC/Jacc diversity for only those days with paired samples from the same frogs

# Untreated pairwise combinations
# Preserve row names
meta_u$SampleID <- rownames(meta_u)

# Generate unique pairwise combinations of samples for each day
day_pairs_u <- combn(levels(meta_u$FINAL.EXPERIMENT.DAY), 2, simplify = FALSE)

# Mock-mock pairwise combinaion
# Preserve row names for mock-mock samples
meta_m$SampleID <- rownames(meta_m)

# Get unique day/pair combinations for each day
day_pairs_m <- combn(levels(meta_m$FINAL.EXPERIMENT.DAY), 2, simplify = FALSE)



#Functions for pairwise comparions

#1. Function for dynamic blocking - Bray-Curtis Untreated
pairwise_u <- lapply(day_pairs_u, function(day_pair) {
  
  # 1. Subset for the pair
  keep_pair <- meta_u$FINAL.EXPERIMENT.DAY %in% day_pair
  meta_pair <- meta_u[keep_pair, ]
  
  # 2. Keep only frogs present on BOTH days being compared
  meta_pair <- meta_pair %>%
    group_by(FrogID) %>%
    filter(n() == 2) %>%
    ungroup() %>%
  ...
  # Skip pair if fewer than 2 paired frogs
  if (length(unique(meta_pair$FrogID)) < 2) {
    return(data.frame(
      comparison = paste(day_pair, collapse = " vs "),
      n_frogs    = length(unique(meta_pair$FrogID)),
      F_stat     = NA, R2 = NA, p_value = NA,
      note       = "Fewer than 2 paired frogs"))
  }
  
  # 3. Call sample IDs directly from column and run BC
  sample_ids <- meta_pair$SampleID
  bray_pair  <- as.dist(as.matrix(bray_u)[sample_ids, sample_ids])
  
  # 4. Block permutation within FrogID
  ctrl <- how(blocks = meta_pair$FrogID, nperm = 999)
  
  # 5. Run adonis2
  fit <- adonis2(
    bray_pair ~ FINAL.EXPERIMENT.DAY,
    data = meta_pair,
    permutations = ctrl)
...
# Combine results and apply p-value corrections
pairwise_results_bray_u <- bind_rows(pairwise_u) %>%
  filter(!is.na(p_value)) %>%
  mutate(p_adj = p.adjust(p_value, method = "bonferroni"))

pairwise_results_bray_u

#Uncomment below to export table
#write_csv(pairwise_results_bray_u, "pairwise_results_bray_u.csv")

#2. Function for dynamic blocking - Bray-Curtis Mock-mock
pairwise_m <- lapply(day_pairs_m, function(day_pair) {
  
  keep_pair <- meta_m$FINAL.EXPERIMENT.DAY %in% day_pair
  meta_pair <- meta_m[keep_pair, ]
  ...
  sample_ids <- meta_pair$SampleID
  bray_pair  <- as.dist(as.matrix(bray_m)[sample_ids, sample_ids])
...
# Combine results and correct p values
pairwise_results_bray_m <- bind_rows(pairwise_m) %>%
  filter(!is.na(p_value)) %>%
  mutate(p_adj = p.adjust(p_value, method = "bonferroni"))

pairwise_results_bray_m

#Uncomment line below to save results
#write_csv(pairwise_results_bray_m, "pairwise_results_bray_m.csv")

#Repeat for Jaccard distance
#3. Function for dynamic blocking - Jaccard Untreated
pairwise_jacc_u <- lapply(day_pairs_u, function(day_pair) {
  ...
  sample_ids <- meta_pair$SampleID
  jacc_pair  <- as.dist(as.matrix(jacc_u)[sample_ids, sample_ids])
  
  ctrl <- how(blocks = meta_pair$FrogID, nperm = 999)
  
  fit <- adonis2(
    jacc_pair ~ FINAL.EXPERIMENT.DAY,
    data = meta_pair,
    permutations = ctrl)
...
# Combine results and correct p-values 
pairwise_results_jacc_u <- bind_rows(pairwise_jacc_u) %>%
  filter(!is.na(p_value)) %>%
  mutate(p_adj = p.adjust(p_value, method = "bonferroni"))

pairwise_results_jacc_u
#Uncomment line below to save results
#write_csv(pairwise_results_jacc_u, "pairwise_results_jacc_u.csv")


#3. Function for dynamic blocking - Jaccard Mock-mock

pairwise_jacc_m <- lapply(day_pairs_m, function(day_pair) {
  keep_pair <- meta_m$FINAL.EXPERIMENT.DAY %in% day_pair
  meta_pair <- meta_m[keep_pair, ]
  meta_pair <- meta_pair %>%
    group_by(FrogID) %>%
  ...
  sample_ids <- meta_pair$SampleID
  jacc_pair  <- as.dist(as.matrix(jacc_m)[sample_ids, sample_ids])
  
  ctrl <- how(blocks = meta_pair$FrogID, nperm = 999)
  
  fit <- adonis2(jacc_pair ~ FINAL.EXPERIMENT.DAY,data = meta_pair, permutations = ctrl)
...
# Combine results and correct p-values
pairwise_results_jacc_m <- bind_rows(pairwise_jacc_m) %>%
  filter(!is.na(p_value)) %>%
  mutate(p_adj = p.adjust(p_value, method = "bonferroni"))

pairwise_results_jacc_m
#Uncomment line below to save results
#write_csv(pairwise_results_jacc_m, "pairwise_results_jacc_m.csv")


# Calculate Bray-Curtis and Jaccard distances and PCoA plots for both Untreated and Mock-mock
set.seed(711)
...
p_bray <- ggplot(
  ...
  stat_ellipse(
    aes(
      group = FINAL.EXPERIMENT.DAY,
      color = FINAL.EXPERIMENT.DAY
    ),
    type = "t",
    linewidth = 1,
    level = 0.95
...
ord_jacc_df <- plot_ordination(
  controls_phy,
  jacc_0,
  type = "samples",
  justDF = TRUE)

anyDuplicated(names(ord_jacc_df))
